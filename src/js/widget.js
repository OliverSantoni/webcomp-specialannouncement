...
if (document.querySelectorAll('odh-covid-widget').length) {
    ...
    container = document.querySelectorAll('odh-covid-widget')[0];
    if (container.classList.contains('odh-covid-initialized') == false) {
...
function getWidgetData() {
    ...
    xmlhttp.open('GET', 'https://tourism.opendatahub.bz.it/v1/Article?pagenumber=1&articletype=specialannouncement&origin=webcomp-specialannouncement&odhactive=true&odhtagfilter=covid-infos&langfilter=' + clientLanguage, true);
...
function initSlider() {
    ...
    var idmslider = tns({
        container: container.querySelector(".odh-covid-wrapper"),
        slideBy: 'page',
        autoplay: false,
        autoWidth: false,
        nextButton: container.querySelector('.odh-covid-button-next'),
        prevButton: container.querySelector('.odh-covid-button-prev'),
        loop: false,
        nav: true,
        center: false,
        onInit: container.classList.add('odh-covid-initialized'),
        mouseDrag: true,
        responsive: {
            0: {
                edgePadding: 20,
    ...
    idmslider.events.on('indexChanged', function () {
        ...
        if (currentSlide == totalSlides) {
            ...
            container.querySelector('.odh-covid-button-next').classList.add('active')
        }

        if (currentSlide != 0) {
            container.querySelector('.odh-covid-button-prev').classList.add('active')
        } else {
            container.querySelector('.odh-covid-button-prev').classList.remove('active')
        }
    });
}
...
function renderIdm(entities) {
    var htmlstring = '';
    htmlstring += '<div class="odh-covid-widget-outer-container">';
    htmlstring += '<div class="odh-covid-widget-container odh-covid-container-horizontal">';
    htmlstring += '<div class="odh-covid-wrapper">';
    ...
    htmlstring += '</div>';
    // PREV ARROW
    htmlstring += '<div class="odh-covid-button-prev" tabindex="0" role="button" aria-label="Previous slide"  aria-disabled="false"><svg height="17.64" viewBox="0 0 9.84 17.64" width="9.84" xmlns="http://www.w3.org/2000/svg"> <defs>  <style> .odh-covid-widget .a-arrow  {  fill: ' + clientColor + ';    }      </style>  </defs>  <path class="a-arrow" d="M5389.72,253.56l8.22,8.28.48.54-.48.54-8.22,8.28-1.14-1.14,7.74-7.68-7.74-7.68"  transform="translate(5398.42 271.2) rotate(180)"></path> </svg></div>';
    // NEXT ARROW
    htmlstring += '<div class="odh-covid-button-next active" tabindex="0" role="button" aria-label="Next slide" aria-disabled="false"><svg height="17.64" viewBox="0 0 9.84 17.64" width="9.84" xmlns="http://www.w3.org/2000/svg">  <defs> <style> .odh-covid-widget .a-arrow  {  fill: ' + clientColor + '; }  </style> </defs>   <path class="a-arrow"  d="M5389.72,253.56l8.22,8.28.48.54-.48.54-8.22,8.28-1.14-1.14,7.74-7.68-7.74-7.68"   transform="translate(-5388.58 -253.56)"></path>   </svg></div>';
    htmlstring += '</div>';
    //powered by Suedtirolinfo
    htmlstring += '<div class="odh-covid-powered-by"> powered by <svg xmlns="http://www.w3.org/2000/svg" width="93.663" height="73.688" viewBox="0 0 93.663 73.688"><path d="M0,64.491V3.353l1.222-.137C1.508,3.184,30.012,0,46.838,0,65.7,0,92.187,3.185,92.453,3.217l1.21.147V64.491l-46.832,9.2Z" fill="#d0d7d8"/><path d="M46.976,1.513c19,0,45.449,3.207,45.449,3.207V63.5L46.969,72.424,1.513,63.5V4.72S30.031,1.513,46.976,1.513Z" transform="translate(-0.138 -0.138)" fill="#fff"/><g transform="translate(2.925 47.664)"><path d="M3.217,65.2V67.37l7.871,1.546,1.523-3.806-1.523-3.806Z" transform="translate(-3.217 -53.236)" fill="#50742f"/><path d="M15.237,59.476l-3.362,1.663V68.75l10.046,1.973,1.251-5.464-1.251-5.783Z" transform="translate(-4.004 -53.069)" fill="#a9cde9"/><path d="M27.642,62.412l-2.249-2.936H22.925V70.723l4.717.926,1.264-5.3Z" transform="translate(-5.008 -53.069)" fill="#b31939"/><path d="M32.449,71.959l1.185-9.223-1.185-9.224-2.381,1.265L28.114,61.87v9.238Z" transform="translate(-5.48 -52.527)" fill="#de7000"/>';
